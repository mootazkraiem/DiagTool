...
namespace CANvision.Native.Converters
{
    ...
    // Turns a 0-100 progress double into a StrokeDashArray for a 70-diameter ring (circumference ≈ 220).
    public sealed class ProgressToArcDashConverter : IValueConverter
    {
        public static readonly ProgressToArcDashConverter Instance = new();
        private const double Circumference = 219.9;

        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            var pct = value is double d ? d : 0;
            var filled = Math.Max(0, Math.Min(100, pct)) / 100.0 * Circumference;
            return new DoubleCollection { filled, Circumference };
        }
