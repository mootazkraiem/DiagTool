...
The project was carried out as a final-year engineering internship (\gls{pfe}). It was framed as an individually driven technical project: the student was responsible for the full life cycle of CANvisionNative, from requirements to design, implementation, and validation, with periodic guidance from an academic supervisor at ESPRIT.
...
\textbf{Product backlog.} The project's internal roadmap (\texttt{ROADMAP.md}) served as a living backlog, listing milestones M1 through M8 (live foundation, hardware layer, multi-state \gls{ml} training, \gls{ai} explanation engine, real-time graphs, session recording, integration, offline/live decode pipeline) in priority order, re-ordered as earlier milestones revealed what the later ones actually needed.
