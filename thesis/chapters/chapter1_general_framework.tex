...
\textbf{First}, the \gls{can} bus, standardized as ISO~11898~\cite{iso11898}, has no built-in security. Any device connected to the bus can send frames, and no frame carries a sender identity or a signature. An intrusion detection system is therefore one of the only practical ways to add a security layer to an existing vehicle without redesigning its electrical architecture.
...
The general objective of this project is to design, implement, and rigorously validate a multi-layer intrusion detection system for vehicle \gls{can} bus networks that adapts to the specific vehicle being analysed and explains its own alerts in plain language.
