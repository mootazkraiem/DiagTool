...
\gls{canfd} extends classic \gls{can} with two changes: the data payload can go up to 64 bytes instead of 8, and the data phase of the frame can run at a higher bit rate than the arbitration phase. \gls{canfd} is increasingly used on newer vehicle platforms, and one of the raw \gls{dbc} files used in this project (for the Kia EV6, Section~\ref{sec:impl-vehicle}) is itself named for an FD bus. The detection pipeline described in Chapter~\ref{chap:design}, however, works on a fixed 8-byte payload throughout its feature-engineering and scoring code, matching the classic \gls{can} frames actually present in every recording used for training and validation (Chapter~\ref{chap:validation}).
...
Table~\ref{tab:attack-comparison} maps each of these patterns to the detection signal it primarily disturbs. No single signal covers every row, which is the main argument, developed in Chapter~\ref{chap:design}, for combining several of them into one fusion score rather than relying on any one of them alone.

\begin{table}[H]
\centering
\caption{CAN bus attack patterns and the detection signal each one primarily disturbs.}
\label{tab:attack-comparison}
\begin{tabularx}{\textwidth}{@{}lXl@{}}
\toprule
\textbf{Attack pattern} & \textbf{Typical effect on the bus} & \textbf{Primary detection signal} \\
\midrule
