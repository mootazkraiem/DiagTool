...
Figure~\ref{fig:validation-workflow} shows the same three passes as a workflow diagram, including the explicit decision point that governed the whole phase: a defect was only fixed once it had been reproduced and explained, never on suspicion alone.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners, minimum width=4.6cm, minimum height=1cm, align=center, font=\small},
    decision/.style={draw, diamond, aspect=2.4, minimum width=3.4cm, align=center, font=\small},
    >=Stealth, node distance=0.9cm
]
\node[box, fill=blue!8] (audit) {Pass 1\\Interactive UI audit};
\node[box, fill=orange!8, below=of audit] (root) {Pass 2\\Root-cause analysis\\(100\% of alerts explained)};
\node[decision, below=of root] (dec) {Reproducible\\evidence?};
\node[box, fill=gray!10, right=2.2cm of dec] (skip) {Not a defect\\— leave unchanged};
\node[box, fill=green!8, below=of dec] (fix) {Pass 3\\Fix + re-validate\\with the same evidence};
\node[box, fill=purple!10, below=of fix] (e2e) {Full end-to-end re-run\\(import $\to$ replay $\to$ detection\\$\to$ AI $\to$ export)};

\draw[->] (audit) -- (root);
\draw[->] (root) -- (dec);
\draw[->] (dec) -- node[left,font=\scriptsize]{yes} (fix);
\draw[->] (dec) -- node[above,font=\scriptsize]{no} (skip);
\draw[->] (fix) -- (e2e);
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    ev/.style={draw, rounded corners, minimum width=9.6cm, minimum height=0.85cm, align=left, font=\small, inner sep=6pt},
    lbl/.style={font=\scriptsize\bfseries, text width=1.6cm, align=right}
]
\node[ev, fill=blue!8]   (a) {Root-cause analysis: 100\% of 15{,}945 baseline alerts explained};
\node[lbl, left=0.2cm of a] {Analysis};
\node[ev, fill=orange!8, below=of a] (b) {Fix 1 — vehicle model selection};
\node[lbl, left=0.2cm of b] {Fix 1};
\node[ev, fill=orange!8, below=of b] (c) {Fix 2 — driving-state classification};
\node[lbl, left=0.2cm of c] {Fix 2};
\node[ev, fill=orange!8, below=of c] (d) {Fix 3 — reason attribution (6 occurrences)};
\node[lbl, left=0.2cm of d] {Fix 3};
\node[ev, fill=orange!8, below=of d] (e) {Fix 4 — replay parser consistency};
\node[lbl, left=0.2cm of e] {Fix 4};
\node[ev, fill=green!8, below=of e]  (f) {End-to-end re-validation in the live application};
\node[lbl, left=0.2cm of f] {Re-test};
\node[ev, fill=red!8, below=of f]    (g) {Fix 5 — alert-polling timer left stopped};
\node[lbl, left=0.2cm of g] {Fix 5};
\node[ev, fill=red!8, below=of g]    (h) {Fix 6 — AI explanation cache staleness};
\node[lbl, left=0.2cm of h] {Fix 6};
\node[ev, fill=green!8, below=of h]  (i) {Final clean build and full end-to-end re-validation};
\node[lbl, left=0.2cm of i] {Release};
