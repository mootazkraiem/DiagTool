...
class MockSignals:
    ...
    def update_phase(self) -> None:
        cycle = self.elapsed_seconds % 90
        # At cycle boundary (not t=0), partially restore SOC and cool temps
        if self.elapsed_seconds > 0 and cycle == 0:
            self.state["SOC"] = self.clamp(self.state["SOC"] + random.uniform(20.0, 35.0), 35.0, 95.0)
            self.state["MotorTemp"] = self.clamp(self.state["MotorTemp"] - random.uniform(8.0, 18.0), 20.0, 65.0)
            self.state["BatteryTemp"] = self.clamp(self.state["BatteryTemp"] - random.uniform(3.0, 10.0), 20.0, 45.0)
            self.state["InverterTemp"] = self.clamp(self.state["InverterTemp"] - random.uniform(5.0, 14.0), 20.0, 55.0)
        if cycle < 10:
            self.phase = 0   # idle / startup
        elif cycle < 50:
            self.phase = 1   # city driving
        elif cycle < 70:
            self.phase = 2   # highway
        else:
            self.phase = 3   # highway stress
    ...
    def export_dataset(self, seconds: int = 80, output_path: str = "data/uds/mock_signals.json") -> None:
    ...
    def generate(self) -> dict:
        ...
        if self.phase == 0:
            self.state["VehicleSpeed"] = self.clamp(base_speed * 0.6 + random.uniform(-0.5, 0.5), 0.0, 3.0)
            self.state["BatteryCurrent"] = self.clamp(base_current * 0.5 + random.uniform(-1.0, 0.5), 2.0, 12.0)
            self.state["SOC"] = self.clamp(base_soc - random.uniform(0.01, 0.03), 30.0, 100.0)
            self.state["BatteryTemp"] = self.clamp(base_bat_temp + random.uniform(-0.1, 0.05), 20.0, 40.0)
            self.state["MotorTemp"] = self.clamp(base_motor_temp + random.uniform(-0.2, 0.05), 20.0, 55.0)
            self.state["InverterTemp"] = self.clamp(base_inv_temp + random.uniform(-0.15, 0.05), 20.0, 50.0)
            self.state["CellVoltageDiff"] = self.clamp(cell_diff + random.uniform(-0.005, 0.001), 0.01, 0.08)
            self.state["DriveMode"] = "IDLE"
            self.state["AcceleratorPedal"] = self.clamp(self.noise(self.state["AcceleratorPedal"], 1.0), 0.0, 2.0)
            self.state["BrakePedal"] = self.clamp(self.noise(self.state["BrakePedal"], 0.5), 0.0, 2.0)
            self.state["RegenerativeBraking"] = 0.0
        elif self.phase == 1:
            target_speed = random.uniform(30, 70)
            self.state["VehicleSpeed"] = self.clamp(base_speed + (target_speed - base_speed) * 0.08 + random.uniform(-1.5, 1.5), 0.0, 80.0)
        ...
        if self.phase == 0:
            self.state["Gear"] = "P"
            self.state["ParkingBrake"] = True
        else:
            self.state["Gear"] = "D" if self.state["VehicleSpeed"] > 0.5 else "N"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", "-o", default="data/uds/mock_signals.json", help="Output file path for exported dataset.")
