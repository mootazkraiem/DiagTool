...
public sealed class VehicleDataService : ObservableObject
{
    ...
    public void Start()
    {
        ...
        logger.Info("[VDS] Started.");
        _ = pythonApiClient.StartLiveSessionAsync(CancellationToken.None);
    }
    ...
    public void Stop()
    {
        ...
        _ = pythonApiClient.StopLiveSessionAsync(CancellationToken.None);
    }
    ...
    private async void OnRefreshTick(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            // Pull live alerts and push to the IDS alert feed when new ones arrive.
            var alertResp = await pythonApiClient.GetLiveAlertsAsync(CancellationToken.None);
            if (alertResp?.Items is { } items && items.Count != lastLiveAlertCount)
            {
            ...
            if (signalPollCounter % 5 == 0)
            {
                ...
                if (sigResp?.Items is { Count: > 0 } sigItems)
                {
                    ...
                    dispatcher.BeginInvoke(() => LiveFramesUpdated?.Invoke(snapshot));
                }
            }
        }
        catch (Exception ex)
